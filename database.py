...
class Database:
    ...
    def update_total_setu(self, ttime: int, num: int):
        ...
        db_conn.commit()
        db_conn.close()
    def update_contrib(self, qqid: int):
        db_conn = sqlite3.connect(os.path.join(
            self.db_path, "setu.db"))
        self.db = db_conn.cursor()
        today = time.strftime("%m%d")
    ...
    def update_total_contrib(self, ttime: int, num: int):
        db_conn = sqlite3.connect(os.path.join(
            self.db_path, "setu.db"))
        self.db = db_conn.cursor()
        today = time.strftime("%m%d")
        sql_info = list(self.db.execute(
            "SELECT time, day_contrib, total_contrib, last_day FROM Total_Data ORDER BY total_contrib DESC,last_day DESC,time DESC limit 0,5"))
        mem_exists = (len(sql_info) != 0)
        if mem_exists:
            day_contrib, total_contrib, last_day = sql_info[0][1:]
        else:
            day_contrib, total_contrib, last_day = 0, 0, ""
        if today != last_day:
            day_contrib = 0
        sql_info = list(self.db.execute(
            "SELECT time, day_contrib, total_contrib, last_day FROM Total_Data WHERE time=?", (ttime ,)))
        mem_exists = (len(sql_info) == 1)
        day_contrib += num
        total_contrib += num
        last_day = today
        print(ttime, mem_exists)
        if mem_exists:
            self.db.execute("UPDATE Total_Data SET day_contrib=?, total_contrib=?, last_day=? WHERE time=?",
                       (day_contrib, total_contrib, last_day, ttime))
        else:
            self.db.execute("INSERT INTO Total_Data (time,day_setu,day_contrib,day_msg,total_setu,total_contrib,total_msg,last_day) VALUES(?,?,?,?,?,?,?,?)",
                       (ttime, 0, day_contrib, 0, 0, total_contrib, 0, last_day))
        db_conn.commit()
        db_conn.close()